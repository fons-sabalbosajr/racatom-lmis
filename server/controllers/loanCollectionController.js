  ...
  try {
    ...
    const formattedCollections = collections.map(collection => {
      ...
      for (const key in obj) {
        ...
        if (obj[key] && typeof obj[key].toString === 'function' && obj[key].constructor.name === 'Decimal128') {
    ...
    res.status(200).json({ success: true, data: formattedCollections, total });
  } catch (error) {
...
export const updateCollection = async (req, res) => {
  try {
    const { id } = req.params;
    const updatedCollection = await LoanCollection.findByIdAndUpdate(id, req.body, { new: true });
