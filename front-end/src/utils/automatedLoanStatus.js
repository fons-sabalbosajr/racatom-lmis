...
export function getAutomatedLoanStatusDetailed({
  ...
  else if (typeof lastCollectionDate === "string")
  ...
  if (
    !lastCollectionDate ||
    daysSince(lastCollectionDate, currentDate) >= t.dormantDays
  ) {
    return {
      status: "DORMANT",
  ...
  if (maturityDate) {
    const litigationAfter = addDays(
      maturityDate,
      t.litigationDaysAfterMaturity,
    );
  ...
  if (lastCollectionDate) {
    ...
    if (pm === "DAILY" && days >= t.arrearsDailyDays) {
      return {
        status: "ARREARS",
        reason: `No collection after ${t.arrearsDailyDays} day${t.arrearsDailyDays === 1 ? "" : "s"} (DAILY)`,
      };
    }
    if (pm === "WEEKLY" && days >= t.arrearsWeeklyDays) {
      return {
        status: "ARREARS",
        reason: `No collection after ${t.arrearsWeeklyDays} day${t.arrearsWeeklyDays === 1 ? "" : "s"} (WEEKLY)`,
      };
    }
    if (
      (pm === "SEMI-MONTHLY" || pm === "SEMI-MOTHLY") &&
      days >= t.arrearsSemiMonthlyDays
    ) {
      // handle typo variant
      return {
        status: "ARREARS",
        reason: `No collection after ${t.arrearsSemiMonthlyDays} day${t.arrearsSemiMonthlyDays === 1 ? "" : "s"} (SEMI-MONTHLY)`,
      };
    }
    if (pm === "MONTHLY" && days >= t.arrearsMonthlyDays) {
      return {
        status: "ARREARS",
        reason: `No collection after ${t.arrearsMonthlyDays} day${t.arrearsMonthlyDays === 1 ? "" : "s"} (MONTHLY)`,
      };
    }
  }
